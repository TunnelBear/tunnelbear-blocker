...
PageStore.prototype.logRequest = function(context, result) {
    ...
    if ( c === '' || c === 'a' ) {
        ...
        console.log('BLOCK REQUEST: ' + requestHostname);
        this.hostnameToCountMap[requestHostname] += 0x00000001;
        this.perLoadBlockedRequestCount++;
        var group = result.match(/\(([^)]+)\)/)[1];
        switch (group) {
            case 'social':
...
PageStore.prototype.sendStats = function () {
    ...
    //     this.isLogging = true;
    //     var pageStore = this;
    //     console.log("start logging: " + pageStore.tabId);
    //     var readLogBuffer = function () {
    //         console.log("readLogBuffer");
    //         var logs = µb.logger.readAll(pageStore.tabId);
    //         for (var index = 0; index < logs.length; index++) {
    //             var log = logs[index];
    //             console.log(log);
    //         }
    //     }
    //     setTimeout(readLogBuffer, 5000);
    // }
}
