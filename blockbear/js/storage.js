...
µBlock.loadFilterLists = function(callback) {
    ...
    var applyCompiledFilters = function(path, compiled) {
        ...
        µb.applyCompiledFilters(compiled, entry);
    };
...
µBlock.applyCompiledFilters = function(rawText, entry) {
    ...
    while ( lineBeg < rawEnd ) {
        ...
        lineBeg = staticNetFilteringEngine.fromCompiledContent(rawText, lineBeg, entry.group);
    }
};
...
µBlock.printSelectedFilters = function () {
    for (var name in this.remoteBlacklists) {
        var item = this.remoteBlacklists[name];
        if (!item.off || item.off == false) {
            console.log(item);
        }
    }
};
